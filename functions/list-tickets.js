export async function onRequestPost(context) {
  ...
  try {
    ...
    const graphqlBody = JSON.stringify({
      query: `query { issues(filter: { project: { id: { eq: "${projectId}" } }, labels: { id: { in: ["${LABEL_VISU_CLIENT}"] } } }, orderBy: updatedAt, first: 50) { nodes { id identifier title description priority createdAt labels { nodes { name color } } } } }`
    });
